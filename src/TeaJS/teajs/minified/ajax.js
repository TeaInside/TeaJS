...
ch.open(data.method,data.url);ch.setRequestHeader("X-Requested-With","XMLHttpRequest");if(typeof data.header==="object"){let x;for(x in data.header){ch.setRequestHeader(x,data.header[x])}}
ch.send(data.data)}